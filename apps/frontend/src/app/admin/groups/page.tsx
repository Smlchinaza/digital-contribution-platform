...
export default function AdminGroupsPage() {
  ...
  async function addUserToGroup(groupId: string, userId: string, position?: number) {
    try {
      const updated = await apiService.addUserToGroup(groupId, userId, position);
  ...
  async function removeUserFromGroup(groupId: string, userId: string) {
    try {
      const updated = await apiService.removeUserFromGroup(groupId, userId);
      setGroups((gs) => gs.map((g) => g.id === groupId ? updated : g));
  ...
  async function assignNextPayout(groupId: string, userId: string) {
    try {
      const updated = await apiService.assignNextPayout(groupId, userId);
      setGroups((gs) => gs.map((g) => g.id === groupId ? updated : g));
  ...
  function getAvailableUsers(group: Group) {
    ...
    return users.filter(user => !groupUserIds.includes(user.id.toString()));
  }
    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {groups.map((g) => {
          ...
          const nextPayoutUser = nextPayout ? users.find(u => u.id.toString() === nextPayout.userId) : null;
...
function UserManagementModal({ 
  ...
  users: User[];
  availableUsers: User[];
  onAddUser: (groupId: string, userId: string, position?: number) => void;
  ...
  const handleAddUser = () => {
    if (selectedUser) {
      onAddUser(group.id, selectedUser, selectedPosition);
      setSelectedUser('');
...
function PayoutAssignmentModal({ 
  ...
  group: Group;
  users: User[];
  onAssignPayout: (groupId: string, userId: string) => void;
  onClose: () => void;
}) {
  const [selectedUser, setSelectedUser] = useState('');
  ...
  const handleAssignPayout = () => {
    if (selectedUser) {
      onAssignPayout(group.id, selectedUser);
      setSelectedUser('');
    }
  };
